...
export const usePagesStore = defineStore("pages", () => {
  ...
  async function createPage(payload?: {
    ...
    pages.value.push({
      ...data,
      _count: { children: 0, blocks: data.blocks?.length || 0 },
    });
    return data as PageFull;
  }
  ...
  async function updatePage(
    id: string,
    payload: Partial<Pick<PageMeta, "title" | "icon" | "isPublished">>,
  ) {
  ...
  async function createBlock(
    ...
    const { data } = await api.post(`/blocks/${pageId}`, {
      type,
      content,
      order,
    });
    if (currentPage.value?.id === pageId) {
      currentPage.value.blocks = [...currentPage.value.blocks, data].sort(
        (a, b) => a.order - b.order,
      );
    }
